...
**重要提示**：
- ⚠️ **必须设置 `ADMIN_PASSWORD`**：这是访问后台管理的密码，不设置将无法登录
- ✅ **请设置强密码**：密码应包含字母、数字和特殊字符
- ✅ **妥善保管密码**：不要将密码提交到版本控制系统
...
项目提供了一个诊断页面，可以快速检查登录问题：

```
访问：http://your-domain.com/admin/login/debug
```

该页面会自动测试：
- ✓ ADMIN_PASSWORD 是否已设置
- ✓ API 端点是否正常工作
- ✓ Cookie 支持是否启用
- ✓ HTTPS 配置
...
2. **重新尝试登录，观察控制台输出**
   ...
   ```
   [登录] 开始登录流程...
   [登录] 发送登录请求到 /api/admin/login
   [登录] 收到响应，状态码: 200
   [登录] 响应数据: {success: true, message: "登录成功"}
   [登录] 登录成功，准备跳转...
   [登录] 执行跳转到 /admin
   ```
...
3. **根据控制台输出诊断问题**
   ...
   - 如果看到 "ADMIN_PASSWORD 未设置" 或 500 错误
     → 说明环境变量未配置或配置错误
   ...
   - 如果没有任何输出
     → 说明 JavaScript 可能有错误，检查是否有红色错误信息
   
   - 如果卡在某一步不继续
     → 查看网络请求是否成功（切换到 Network 标签）

4. **检查网络请求**
   
   在开发者工具的 "Network（网络）" 标签中：
   - 找到 `/api/admin/login` 请求
   - 查看状态码（应该是 200）
   - 查看响应内容
   - 查看响应头中的 Set-Cookie

#### 问题：输入密码后只刷新页面，不跳转到后台

**原因**：
1. `ADMIN_PASSWORD` 环境变量未设置
2. 密码错误
3. Cookie 设置失败

**解决方案**：

1. **检查 .env 文件是否设置了 ADMIN_PASSWORD**
   ```bash
   cd /www/wwwroot/hsfzfx
...
3. **检查 PM2 日志查看错误**
   ```bash
   pm2 logs hsfx-site --lines 50
   ```
   
   如果看到 "ADMIN_PASSWORD 未设置" 错误，说明环境变量没有生效。
...
5. **在生产环境中确保使用 HTTPS**
   - HTTP 环境下 `secure` cookie 可能有问题
   - 建议配置 SSL 证书

#### 问题：显示"ADMIN_PASSWORD 未设置"错误
